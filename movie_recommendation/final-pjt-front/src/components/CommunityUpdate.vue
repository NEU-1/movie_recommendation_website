    ...
    <div class="row justify-content-center">
      <div class="col-12 col-md-8">
        <form @submit.prevent="updateCommunity">
          <div class="mb-3">
            <input v-model.trim="newTitle" type="text" class="form-control" placeholder="제목" />
          </div>
          <div class="mb-3">
            <textarea v-model.trim="newContent" rows="20" class="form-control" placeholder="내용을 입력하세요."></textarea>
          </div>
        ...
        <div class="d-grid gap-2 mt-3">
          <router-link :to="{ name: 'CommunityDetail', params: { community_pk: communityId }}" class="btn btn-secondary">뒤로가기</router-link>
        </div>
      </div>
    </div>
  </div>
